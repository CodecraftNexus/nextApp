...
const Horoscope = () => {
  ...
  useEffect(() => {
    ...
    const allLoaded = requiredPlanets.every(
      (planet) => predictions[planet]?.result?.data?.predictions
    );
  ...
  const Navam = navam.result.data;
  const astroData = astro.result.data;
  ...
  const boxesnavam: Record<number, string[]> = {
    1: Navam.box1 || [],
    2: Navam.box2 || [],
    3: Navam.box3 || [],
    4: Navam.box4 || [],
  ...
  const indexsesnavm: Record<number, Record<number, string>> = {};
  Object.entries(boxesnavam).forEach(([boxNum, navam]) => {
    const num = parseInt(boxNum);
    indexsesnavm[num] = {};
    navam.forEach((n, i) => {
      indexsesnavm[num][i + 1] = n;
    });
  });

    const displayItems = [
    { label: "නැකත", value: astroData.nakshatra },
    { label: "පාදය", value: `${astroData.nakshatra_pada} පාදය` },
    { label: "තිතිය", value: astroData.tithi },
    { label: "රාශිය", value: astroData.rasi },
    { label: "ගණය", value: astroData.gana },
    { label: "යෝනිය", value: astroData.yoni },
    { label: "වස්‍යය", value: astroData.vasya },
    { label: "නාඩිය", value: astroData.nadi },
    { label: "වර්ණය", value: astroData.varna },
    { label: "පායා", value: astroData.paya },
    { label: "තත්වය", value: astroData.tatva },
    { label: "ජීවන මැණික", value: astroData.life_stone },
    { label: "වාසනා මැණික", value: astroData.lucky_stone },
    { label: "ධන මැණික", value: astroData.fortune_stone },
    { label: "නම ආරම්භය", value: astroData.name_start },
    { label: "ලග්නය", value: astroData.ascendant_sign },
    { label: "ලග්න නැකත", value: astroData.ascendant_nakshatra },
    { label: "රාශි අධිපතිය", value: astroData.rasi_lord },
    { label: "නැකත් අධිපතිය", value: astroData.nakshatra_lord },
    { label: "සූර්ය රාශිය", value: astroData.sun_sign },
    { label: "කරණය", value: astroData.karana },
    { label: "යෝගය", value: astroData.yoga }
  ];
    ...
    const predictionContent = Object.entries(predicttionData[0])
      ...
      .map(([key, value]: [string, any]) => ({
        ...
        predictionText: value
          .split(".")
          .filter((sentence: any) => sentence.trim() !== "")
  ...
  return (
    <div className="w-full min-h-screen px-4 py-6 md:py-8">
      <div className="flex flex-col lg:flex-row gap-4 w-full justify-center lg:items-center items-start pb-8 lg:gap-10 font-bold funnel-sans-text text-md shadow mb-4">
        <span className="flex  gap-2 items-center">
          <div className="w-3 h-3 bg-blue-400 rounded-full"></div> Date of Birth
          - {user.dateOfBirth}
        </span>
        <span className="flex  gap-2 items-center">
          <div className="w-3 h-3 bg-blue-400 rounded-full"></div>Time of Birth
          - {user.birthTime}
        </span>
        <span className="flex  gap-2 items-center">
          {" "}
          <div className="w-3 h-3 bg-blue-400 rounded-full"></div>Birth location
          - {user.birthLocation}
        </span>
      </div>
      <div className="mx-auto">
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-4 items-start justify-center">
          <div className="wrapper flex-col gap-14 w-full lg:w-1/2 flex justify-center items-center">
            ...
            <HoroscopeChart
              indexses={indexsesnavm}
              lagnaya={navam?.result?.data?.lagnaya}
              isHoroscope={false}
            />
          </div>
      ...
      <div className="w-full mt-5 mb-5 shadow py-8  rounded-2xl justify-start px-4 lg:px-0 lg:justify-center items-center flex bg-linear-to-r from-purple-600 to-indigo-600">
        <div className="grid lg:grid-cols-4 grid-cols-1  gap-8 justify-center items-center funnel-sans-text text-xl text-white font-bold" >
          {displayItems.map((item, index) => (
          <span key={index} className="flex gap-3 items-center">
            <div className="w-3 h-3 bg-blue-50 rounded-full shrink-0"></div>
      ...
      <div className="group w-full mt-5">
        <div className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 overflow-hidden border border-purple-100">
          ...
          <div className="p-4 sm:p-6">
            <PredictionSections
              predictionTitle="සූර්‍යයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Sun")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="චන්ද්‍රයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Moon")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="බුධ ග්‍රහයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Mercury")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="ශුක්‍ර ග්‍රහයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Venus")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="අඟහරු ග්‍රහයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Mars")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="සෙනසුරු ග්‍රහයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Saturn")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="බ්‍රහස්පති ග්‍රහයාට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Jupiter")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="රාහුට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Rahu")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
            <PredictionSections
              predictionTitle="කේතුට අනුව ඔබගේ බලපෑම"
              predictionContent={getPredictoncontent("Ketu")}
              handleCorrect={handleCorrect}
              handleInCorrect={handleInCorrect}
            />
          </div>
