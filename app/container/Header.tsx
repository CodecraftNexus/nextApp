...
const Header = () => {
  ...
  const { logout, user, isAuthenticated, switchProfile, addProfile ,checkAuth} =
    useAuth();
  ...
  const currentUser: Account = {
    id: user?.id,
    name: user?.name,
    nickname: user?.reference == null ? "me" : user?.nikname,
    isMain: user?.reference == null,
  ...
  const accounts: Account[] = React.useMemo(() => {
    ...
    if (user.reference != null && user.mainUser) {
      accountsList.push({
        id: user.mainUser.id,
        name: user.mainUser.name,
        nickname: user.mainUser.nickname || user.mainUser.nikname || "me",
        isMain: true,
        profileImage: user.mainUser.profileImg,
      });
    }

    accountsList.push(currentUser);
      ...
      user.profiles.forEach((profile: any) => {
        if (profile.id !== user.id) {
          accountsList.push({
            id: profile.id,
            name: profile.name,
            nickname: profile.nikname,
            isMain: false,
            profileImage: profile?.ProfileImages?.[0]?.imagePath,
          });
        }
      });
    }
  ...
  const handleSwitchAccount: any = async (account: Account) => {
    try {
      
      await switchProfile(account.id);
  ...
  const handleAddAccount = async (name: string, nickname: string) => {
    try {
      await addProfile(name, nickname);
    } catch (error) {
