...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const checkAuth = useCallback(async (id?: any) => {
    const now = Date.now();
  ...
  // Switch profile with proper error handling
  const switchProfile = useCallback(
    async (profileId: string) => {
      ...
      try {
        ...
        if (result.success) {
          // Verify and load the new profile
          await checkAuth(profileId);
  ...
  const addProfile = useCallback(
    async (name: string, nickname: string) => {
      ...
      try {
        ...
        if (result.success) {
          // Refresh user data to get updated profiles list
          await checkAuth();
